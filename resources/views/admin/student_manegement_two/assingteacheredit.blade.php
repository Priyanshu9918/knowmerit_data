                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <form action="{{ route('admin.assign_teacher', ['id' => base64_encode($user->id)]) }}"
                                    ...
                                    @php
                                    $cq1 = DB::table('credits')->where('student_id',$user->id)->get();
                                    @endphp
                                @if(isset($cq1))
                                @foreach($cq1 as $key=>$c1)
                                    <div class="row">
                                        <div class="col-md-6">
                                            <label class="form-label"><b>Category</b></label>
                                            <select class="form-control category" id="category{{$c1->id}}" name="category[]">
                                                <option value="">Select Category</option>
                                                @foreach ($parent_categories as $category)
                                                    <option value="{{ $category->id }}"
                                                        @if ($category->id == $c1->class_id) selected @endif>{{ $category->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        @php
                                            ...
                                            ->where('status', 1)
                                            ->where('parent',$c1->class_id)
                                            ->get();
                                        @endphp
                                        <div class="col-md-6" id="sb_n">
                                            <label class="form-label"><b>Sub Category</b></label>
                                            <select class="form-control sub_category" id="sub_category{{$c1->id}}" name="sub_category[]">
                                                <option value="">Select Sub Category</option>
                                                @foreach ($parent_sub as $cat)
                                                    <option value="{{ $cat->id }}"
                                                        @if ($cat->id == $c1->sub_id) selected @endif>{{ $cat->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                            <p style="margin-bottom: 2px;" class="text-danger error_container"
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <select class="form-control teacher" id="tutor_name{{$c1->id}}" name="tutor_name[]">
                                                <option value="">Select Category</option>
                                                @foreach ($teachers as $t)
                                                    <option value="{{ $t->id }}"
                                                        @if ($t->id == $c1->teacher_id) selected @endif>{{ $t->name }}
                                                        ({{ $t->email }})</option>
                                                @endforeach
                                            </select>
                                            <p style="margin-bottom: 2px;" class="text-danger error_container"
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <input type="number" class="form-control" id="credit{{$c1->id}}" name="credit[]"
                                                value="{{ $c1->credit }}" />
                                            <p style="margin-bottom: 2px;" class="text-danger error_container"
                                                id="error-credit"></p>
                                            <input type="hidden" name="student_id1[]" value="{{$c1->id}}">
                                        ...
                                        <div class="card-footer">
                                            <div class="row">
                                                <div class="col-lg-12 text-center">
                                                    <button type="submit" id="update" class="btn btn-success submit mr-2">Update</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                ...
                $(document).ready(function() {
                    ...
                    $("#add1").click(function() {
                        ...
                        $("#mt12").append(`<div class="row gx-3" id="row${i}">
                            <input type="hidden" name="student_id1[]" value="">
                            <div class="col-md-6">
                                            <label class="form-label"><b>Category</b></label>
                                        ...
                                        <div class="col-md-6" >
                                            ...
                                            <label class="form-label"><b>Sub Category</b></label>
                                            <select class="form-control sub_category" name="sub_category[]" id="sub_category${i}">
                                                <option value="">Select Sub Category</option>
                                            </select>
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                if (response.success == true) {
                                    ...
                                    toastr.success("Teacher Assign Updated Successfully");
                                    window.setTimeout(function() {
                                        window.location = "{{ url('/') }}" +
