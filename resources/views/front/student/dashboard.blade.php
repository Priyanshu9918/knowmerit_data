...
{
    border-radius: 0;
    border-bottom: 3px solid #468dcb;
    color: #0094ff;
}
.enqui-task ul li a
{
...
.datetimeli input[type="date"] {
    color: #ff0100;
}
</style>
<div class="col-xl-9 col-lg-8 col-md-12" style="background-color: #4f94cf12; border-radius: 10px;">

    <div class="course-group mb-0 d-flex mt-2 mb-2" style="background-color:#76aad821; padding:20px; border-radius: 10px;">
...
<!-- <div class="category-tab tickets-tab-blk enqui-task enqui12">
    <ul class="nav nav-justified ">
        <li class="nav-item"><a href="#upcoming" class="nav-link active" data-bs-toggle="tab"> <img class="img-fluid v-icon" alt="" src="{{asset('assets/img/my-img/upcoming-one-img.png')}}">Upcoming Class</a>
        </li>
        <li class="nav-item"><a href="#past" class="nav-link enquiri" data-bs-toggle="tab"><i style="margin-right: 5px;
    color: #fcc62a;" class="fa fa-backward" aria-hidden="true"></i> Past Class</a></li>
    </ul>
...
$student_enquiry1 = App\Models\BookSession::where('student_id',auth()->user()->id)->where('end_time' ,'<',$dt1)->orderBy('start_time','ASC')->get();
    $user_data = App\Models\BookSession::where('student_id', auth()->user()->id)->where('end_time' ,'<',$dt1)->orderBy('id', 'Desc')->pluck('teacher_id');
        ...
        <div class="tab-content">
            ...
            <span class="noupcom">There is no upcoming session </span>
                ...
                <div class="settings-top-widget ">
                    ...
                    <div class="row">
                        <div class="col-md-12 for_demo forscoll-des">
                                    ...
                                    <div class="comman-space pb-0 shadow-none">
                                        <div class="settings-tickets-blk course-instruct-blk table-responsive">
                                            <table class="table table-nowrap mb-0">
                                                <tbody>
                                                    @foreach ($student_enquiry1 as $st212)
                                                    @php
                                                    $st31 =
                                                    DB::table("tutors")->where('user_id',$st212->teacher_id)->first();
                                                    if(isset($st31)){
                                                    $avtar = DB::table('users')->where('id', $st31->user_id)->where('status', 1)->first();

                                                    $avtar1 = DB::table('users')->where('id', Auth::user()->id)->where('status', 1)->first();
                                                    $timezone = DB::table('time_zones')->where('id',$avtar1->timezone ?? 195)->first();

                                                    $today123 = date("Y-m-d h:i A");

                                                    $time_from_t145 = new \DateTime($today123, new \DateTimeZone('UTC'));
                                                    ...
                                                    $time_from_t12 = new \DateTime($st212->end_time, new \DateTimeZone('UTC'));
                                                    $time_from_t12->setTimezone(new \DateTimeZone($timezone->timezone));
                                                    $e_time12 = $time_from_t12;

                                                    $cat1 = DB::table('categories')->where('id',$st212->class_id)->first();
                                                    }
                                                    @endphp
                                                    @if(isset($st31))
                                                    <tr style="border-bottom: 1px solid #e5dede;" class=" bg-white">
                                                        <div class="tab12">
                                                                ...
                                                                <div class="sell-table-group d-flex align-items-center">
                                                                    <div class="sell-group-img student-news">
                                                                        <a href="#">
                                                                            @if(isset($cat1))
                                                                            @if($cat1->image)
                                                                            <a href=""><img src="{{ asset('uploads/categories/' . $cat1->image) }}" class="img-fluid s-list" alt=""></a>
                                                                            @else
                                                                            <a href=""><img src="{{ asset('assets/img/my-img/web_img/10.png') }}" class="img-fluid s-list" alt=""></a>
                                                                            @endif
                                                                            @endif </a>
                                                                    </div>
                                                                    <div class="sell-tabel-info">
                                                                        <div style="font-size: 20px;display: flex;"><a href="javascript:void(0)" id="student2" data-id="{{$st212->teacher_id}}" data-cl="{{$cred->class_id ?? 1}}" data-sub="{{$cred->sub_id ?? 2}}">{{$cat1->name ?? ''}}</a></div>
                                                                        @if(isset($time1))<span>{{date('M d, Y h:i:sa', strtotime($time1))}}</span>@endif
                                                            ...
                                                            <!-- <td style="float: right;"> <a href="{{$st212->student_url}}"><span class="badge info-low">Join Class</span> </a> </td> -->
                                                            @if($st212->is_cancelled == 1)
                                                            <td style="text-align: center;"><span class="badge info-low" style="color: #fff;border-radius: 7px!important;">Cancelled</span></td>
                                                            @else
                                                            @if(isset($e_time12))
                                                            @if($e_time12 < $today1)<td style="text-align: center;">
                                                                <span class="badge info-low" style="color: #fff;border-radius: 7px!important;">Completed</span></td>
                                                                @endif
                                                                @endif
                                                            @endif
                                                        </div>
            ...
            <div class="tab-pane fade active show" id="upcoming">
                ...
                $student_enquiry = App\Models\BookSession::where('student_id',auth()->user()->id)->where('end_time' ,'>',$dt)->where('student_url','<>',null)->orderBy('start_time','ASC')->get();
                    $user_data1 = App\Models\BookSession::where('student_id', auth()->user()->id)->where('end_time' ,'>',$dt)->orderBy('id', 'Desc')->pluck('teacher_id');
                    $us12e1 = DB::table('users')->whereIn('id',$user_data1)->where('status', 1)->get();
                    @endphp
                    @if(count($us12e1) > 0)
                    <div class="settings-top-widget ">
                        <div class="row">
                            <div class="top-headingg">
                                <h3>Your upcoming Session</h3>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-12 for_demo forscoll-des">
                                    ...
                                    <div class="settings-inner-blk p-0 bg-transparent">
                                        <div class="comman-space pb-0 shadow-none">
                                            <div class="settings-tickets-blk course-instruct-blk table-responsive">
                                                <table class="table table-nowrap mb-0">
                                                    <tbody>
                                                        @foreach ($student_enquiry as $key=>$st2)
                                                        @php
                                                        $st3 = DB::table("tutors")->where('user_id',$st2->teacher_id)->first();
                                                        if(isset($st3)){
                                                        $avtar = DB::table('users')->where('id', $st3->user_id)->where('status', 1)->first();
                                                        ...
                                                        $time_from_t12 = new \DateTime($st2->start_time, new \DateTimeZone('UTC'));
                                                        $time_from_t12->setTimezone(new \DateTimeZone($timezone->timezone));
                                                        $time1123 = $time_from_t12->format("Y-m-d h:i A");

                                                        $cat = DB::table('categories')->where('id',$st2->class_id)->first();

                                                        $ex_time1 = Carbon\Carbon::now()->addHour(3);
                                                        ...
                                                        <tr style="border-bottom: 1px solid #e5dede;" class=" bg-white">
                                                            <div class="tab12">
                                                                <td>
                                                                    <div class="sell-table-group d-flex align-items-center">
                                                                        <div class="sell-group-img student-news">
                                                                            @if(isset($cat))
                                                                            @if($cat->image)
                                                                            <a href=""><img src="{{ asset('uploads/categories/' . $cat->image) }}" class="img-fluid s-list" alt=""></a>
                                                                            @else
                                                                            <a href=""><img src="{{ asset('assets/img/my-img/web_img/10.png') }}" class="img-fluid s-list" alt=""></a>
                                                                            @endif
                                                                            @endif
                                                                        ...
                                                                        <div class="sell-tabel-info">
                                                                            <div style="font-size: 20px;display: flex;"><a href="javascript:void(0)" id="student1" data-id="{{$st2->teacher_id}}" data-cl="{{$cred->class_id ?? 1}}" data-sub="{{$cred->sub_id ?? 2}}">{{$cat->name ?? ''}}</a></div>
                                                                            <span>{{date('M d, Y h:i:sa', strtotime($time1123))}}</span>
                                                                ...
                                                                <!-- <td style="float: right;"> <a href="{{$st2->student_url}}"><span
                                                class="badge info-low">Join Class</span> </a> </td> -->
                                                                @if($st2->is_cancelled == 1)
                                                                <td style="text-align: center;"><span class="badge info-low" style="color: #fff;border-radius: 7px!important;">Cancelled</span></td>
                                                                @else
                                                                @if($time1 <= $today)<td style="text-align: center;">
                                                                    <a href="{{$st2->student_url}}"><span class="badge info-low" style="color: #fff;border-radius: 7px!important;background-color: #009fff;">Join Now</span></a></td>
                                                                    @else
                                                                    <td style="text-align: center;" id="demo1{{$st2->id}}"> @if($ex_t_date <= $time1)<i id="cancel-reshed" data-id="{{$st2->id}}" class="fa fa-times-circle close-v" style="font-size:24px;color: #ff0909;"></i>@endif<span class="badge info-low" style="color: #fff;background-color: #f96d41;border-radius: 7px!important;">Starts in <span id="demo{{$st2->id}}">
                                                                                </span></span></td>
                                                                    @endif
                                                                    @endif
                                                            </div>
                                                            <!-- <script>
                                                                ...
                                                                var x {
                                                                    ...
                                                                    if (distance < 0) {
                                                                        ...
                                                                        document.getElementById('demo1{{$st2->id}}').innerHTML = '<a href="{{$st2->student_url}}"><span class="badge info-low" style="color: #fff;border-radius: 7px!important;background-color: #009fff;">Join</span></a>';
                                                                    }
                                                                }, 1000);
                                                            </script> -->
                                                            <script>
                                                                    // Set the date we're counting down to
                                                                var countDownDate{{$st2->id}} = new Date("{{$time1123}}").getTime();
                                                                // Update the count down every 1 second
                                                                var x{{$st2->id}} = setInterval(function() {
                                                                // Get today's date and time
                                                                var now = new Date().getTime();
                                                                // Find the distance between now and the count down date
