...
class StudentController extends Controller
{
    ...
    public function cancleclass(Request $request){
        ...
        try{
            ...
            $date_time  = str_replace('at ','',  Carbon::now());
            $time_from_t1   = new \DateTime($date_time, new \DateTimeZone($tz3));
            $time_from_t1->setTimezone(new \DateTimeZone('UTC'));
            ...
            if($to_time > 24){
                ...
                DB::table('credits')->where('user_id',$book_session->student_id)->update($data1);
            }
...
class StudentController extends Controller
{
    public function index()
    {
        ...
        $tz     = ($tz->users != null && $tz->users->TimeZone != null) ? $tz->users->TimeZone->timezone : 'Europe/Berlin';
        $i      = 0;
        $date   = new \DateTime();
        $date->setTimezone(new \DateTimeZone($tz));
        $cur_date = $date->format('Y-m-d H:i:s');
    ...
    {
        $id = $request->active;
        $video = Db::table('student_videos')->where('teacher_id', $id)->get();
        $homework = Db::table('student_documents')->where('d_type', 'homework')->where('teacher_id', $id)->get();
        $test = Db::table('student_mcqs')->where('teacher_id', $id)->get();
        $document = Db::table('student_documents')->where('d_type', 'document')->where('teacher_id', $id)->get();
