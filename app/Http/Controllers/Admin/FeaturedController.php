...
class FeaturedController extends Controller
{
    public function index(Request $request){
        if($request->ajax())
        {
            ...
            ->addColumn('action', function($row) {
                ...
                $edit_url = url('/admin/featured/edit',['id'=>base64_encode($row->id)]);
            ...
            $datatables = $datatables->rawColumns(['check', 'action','description'])->make(true);
    ...
    public function create(Request $request)
    {
        if ($request->isMethod('get')) {
            $featureds = DB::table('featureds')->where('status',1)->get();
           return view('admin.featured.create',compact('featureds'));
            }
        ...
        try {
            ...
            // dd($data);
            DB::table('featureds')->insert($data);
    ...
    {
        $featureds_id = base64_decode($request->id);

        if($request->isMethod('get'))
        {

            $featureds  = DB::table('featureds')->where('id',$featureds_id)->first();
            return view('admin.featured.edit' ,compact('featureds'));
        }
        ...
        try{
            ...
            DB::table('featureds')->where('id',$featureds_id)->update($data);
