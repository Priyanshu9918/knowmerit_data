...
class BenifitsController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $games = DB::table('benifits')->where('status', '<>', 2)->orderBy('id','DESC')->get();
            $datatables = Datatables::of($games)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $edit_url = url('/admin/benifit/edit', ['id' => base64_encode($row->id)]);
    ...
    public function create(Request $request)
    {
        if ($request->isMethod('get')) {
            $amount = DB::table('member_ship_plans')->where('status', '<>', 2)->get();
            return view('admin.benifit.create',compact('amount'));
        }
...
// dd($data);
    ...
    public function edit(Request $request)
    {
        ...
        if ($request->isMethod('get')) {
            $user  = DB::table('benifits')->where('id', $user_id)->first();
            $amount = DB::table('member_ship_plans')->where('status', '<>', 2)->get();
            return view('admin.benifit.edit', compact('user','amount'));
        }
        ...
        try {
            ...
            DB::table('benifits')->where('id', $user_id)->update($data);
