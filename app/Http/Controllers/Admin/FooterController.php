...
class FooterController extends Controller
{
    public function index(Request $request){
        if($request->ajax())
        {
            ...
            ->addColumn('action', function($row) {
                ...
                $edit_url = url('/admin/footer/edit',['id'=>base64_encode($row->id)]);
    ...
    public function create(Request $request)
    {
        if ($request->isMethod('get')) {
            $footers = DB::table('footers')->where('status',1)->get();
           return view('admin.footer_menu.create',compact('footers'));
            }
    ...
    {
        $footers_id = base64_decode($request->id);

        if($request->isMethod('get'))
        {

            $footers  = DB::table('footers')->where('id',$footers_id)->first();
            return view('admin.footer_menu.edit' ,compact('footers'));
        }
        ...
        try{
            ...
            DB::table('footers')->where('id',$footers_id)->update($data);
